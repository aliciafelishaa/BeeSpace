...
export function ProfileInformation({
    ...
    const [initialUsername, setInitialUsername] = useState(initial?.username ?? "");

    const { control, handleSubmit, setValue, watch, formState: { errors }, clearErrors } = useForm<ProfileInformationValues>({
        defaultValues: {
    ...
    const currentUniversity = watch("universityName");
    const currentMajor = watch("major");
    const currentStudentId = watch("studentId");
    const currentEnrollYear = watch("enrollmentYear");
    const currentGradYear = watch("graduationYear");
    ...
    const handleSaveForm = (data: ProfileInformationValues) => {
        if (!loading) {
            onSave?.(data);
            setIsEditing(false);
        }
    ...
    const handleCancel = () => {
        setValue("avatar", initial?.avatar);
        setValue("username", initial?.username ?? "");
        setValue("fullName", initial?.fullName ?? "");
        setValue("email", initial?.email ?? "");
        setValue("bio", initial?.bio ?? "");
        setIsEditing(false);
        onCancel?.();
    };
            ...
            >
                ...
                {!isEditing ? (
                    <View style={{ marginTop: 32 }}>
                        ...
                        <InfoRow label="Full Name" value={currentFullName} />
                        <InfoRow label="Email" value={currentEmail} />
                        <InfoRow label="Bio" value={currentBio} />
                        ...
                        <InfoRow label="University's Name" value={currentUniversity} />
                        <InfoRow label="Major" value={currentMajor} />
                        <InfoRow label="Student ID" value={currentStudentId} />

                        <View style={{ flexDirection: "row", gap: 20 }}>
                            <View style={{ flex: 1 }}>
                                <InfoRow label="Enrollment Year" value={currentEnrollYear} />
                            </View>
                            <View style={{ flex: 1 }}>
                                <InfoRow
