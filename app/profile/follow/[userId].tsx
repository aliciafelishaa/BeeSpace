import {
  FollowListItem,
  FollowUser,
} from "@/components/profile/FollowListItem";
...
export default function FollowScreen() {
  ...
  const processUserList = async (
    list: Awaited<ReturnType<typeof getFollowersList>>
  ): Promise<FollowUser[]> => {
  ...
  useEffect(() => {
    if (
      initialTab &&
      (initialTab === "followers" || initialTab === "following")
    ) {
  ...
  const handleMessage = async (targetUserId: string) => {
    ...
    try {
      const chatId = await initiateChat(currentUserId, targetUserId);
      console.log("ChatID:", chatId);

      if (chatId) {
        router.push(`/directmessage/chat?id=${chatId}&hostId=${targetUserId}`);
      } else {
      ...
      >
        ...
        >
          <Text
            ...
            style={{
              color:
                activeTab === "followers" ? COLORS.primary : COLORS.neutral500,
            }}
          >
            Followers
          </Text>
        ...
        >
          <Text
            ...
            style={{
              color:
                activeTab === "following" ? COLORS.primary : COLORS.neutral500,
            }}
          >
            Following
          </Text>
