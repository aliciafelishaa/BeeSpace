...
export default function CreateRoomPage() {
    ...
    const handleChange = (key: string, value: any) => {
        if (key === 'date') {
            ...
            if (isToday && (formData.timeStart || formData.timeEnd)) {
                ...
                if (formData.timeEnd && !resetEnd) {
                    const [hours, minutes] = formData.timeEnd.split(':').map(Number)
                ...
                setFormData({
                    ...formData,
                    date: value,
                    timeStart: resetStart ? '' : formData.timeStart,
                    timeEnd: resetEnd ? '' : formData.timeEnd,
                })
            } else {
        ...
        >
            ...
            >
                <View className="mt-12 mb-6 items-center justify-center">
                    <TouchableOpacity
                        onPress={handleBack}
                        className="w-12 h-12 px-2 rounded-full bg-[#FFFFFF] border border-[#F5F5F5] absolute left-5 bottom-2 flex justify-center items-center"
                    >
                        <IconBack />
                    </TouchableOpacity>
                    ...
                    {[1, 2, 3].map((num, index) => (
                        <React.Fragment key={num}>
                            <View className="items-center">
                                <View
                                    className={`w-12 h-12 rounded-full flex items-center justify-center ${step >= num ? "bg-[#FFD661]" : "bg-[#FFF6D5]"
                                        }`}
                                >
                                    <Text
                                        className={`font-medium text-lg ${step >= num ? "text-black" : "text-[#737373]"
                                            }`}
                                    >
                                        {num}
                                    </Text>
                            ...
                            {index < 2 && (
                                <View
                                    className={`flex-1 h-[2px] bottom-3 ${step > num ? "bg-[#FFD661]" : "bg-[#FFF6D5]"
                                        }`}
                                />
                            )}
                        </React.Fragment>
                        ...
                        <View>
                            <View className="mb-5">
                                ...
                                {uploading ? (
                                    ...
                                    <View className="relative w-full h-48 mb-2">
                                        <TouchableOpacity
                                        ...
                                        <View className="absolute bottom-2 right-2 flex-row gap-2" style={{ zIndex: 999 }}>
                                            <TouchableOpacity
                                                onPress={(e) => {
                                                    e.stopPropagation()
                                                    handleRemovePhoto()
                                                }}
                                                className="bg-red-500 p-2 rounded-lg items-center justify-center shadow-lg"
                                                activeOpacity={0.8}
                                                style={{ elevation: 5 }}
                                            >
                                            ...
                                            <TouchableOpacity
                                                ...
                                                className="bg-[#FCBC03] p-2 rounded-lg items-center justify-center shadow-lg"
                                                activeOpacity={0.8}
                                                style={{ elevation: 5 }}
                                            >
